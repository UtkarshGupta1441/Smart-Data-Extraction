...
docseg_model = YOLO("C:\\Users\\utkar\\Downloads\\models 1\\models\\yolov11l-doclaynet.pt")
...
def process_image(image_path):
    ...
    output_dir = os.path.join("output2", base_name)
    ...
    results = docseg_model(source=np.array(img), conf=0.1, iou=0.5)
    label_counts = {}
...
def clear_output2_folder(folder_path="output2", retries=3, retry_delay=0.35):
...
async def take_smart_screenshots(url, output_dir="smart_screenshots", max_scrolls=500):
    os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    url = "https://freedium-mirror.cfd/fa1c7992b195"

    clear_output2_folder("output2")

    # Run Playwright to take screenshots
    screenshots_folder = asyncio.run(take_smart_screenshots(url))

    # Process screenshots with YOLO doclaynet
    print(f"Processing screenshots in folder: {screenshots_folder}")
    process_folder(screenshots_folder)
